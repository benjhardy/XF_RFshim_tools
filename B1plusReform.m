function [B1plus_m, FOX] = B1plusReform(B1plus_m,ver,ix,iy,iz)
    ...
    %        voxelizedMesh (loaded from working dir, not an input) 
    %        -  struct containing the brain and frankMask
    %        Indices - the indices of mesh where the sensor is located
    %        ver - version of code to run, ver 1 is older field maps
    %        (outdated)
    %        2 - if the desired FOX is brain shaped
    %        3 - if the desired FOX is head shaped
